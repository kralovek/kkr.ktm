...
public class ValueParser {
	...
	public static ValuePattern parseValue(String text) throws Exception {
		int phase = 0;
		int iPos = 0;
		int beg = 0;

		Collection<ValueFlag> flags = new ArrayList<ValueFlag>();
		...
		go_for : for (iPos = 0; iPos < text.length(); iPos++) {
			...
			switch (phase) {
				...
				case 2 :
					if (c == '>') {
						String flag = text.substring(beg, iPos);
						try {
							ValueFlag valueFlag = ValueFlag.valueOf(flag);
							flags.add(valueFlag);
						} catch (Exception e) {
		...
		ValuePatternImpl retval = new ValuePatternImpl();
	...
	private static void checkParts(List<Part> pParts) throws Exception {
		...
		} else if (!(pParts.get(0) instanceof Open) || !(pParts.get(pParts.size() - 1) instanceof Close)) {
			throw new Exception("Syntaxe problem");
		}
		...
		if (pObject != null && pObject.getClass().isArray()) {
			...
			if (countArrays != 0) {
				if (countArrays != array.length) {
					throw new Exception("All elements on the level " + pLevel + " must be arrays");
				}
				for (int i = 0; i < array.length; i++) {
