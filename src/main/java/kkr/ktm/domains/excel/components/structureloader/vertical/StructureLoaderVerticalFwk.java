...
public abstract class StructureLoaderVerticalFwk extends StructureLoaderBase {
	...
	public void config() throws ConfigurationException {
		...
		if (_rowCode == null) {
			...
			indexCode = UtilsExcel.adaptAndCheckRowId(getClass(), _rowCode, "rowCode");
		}
		if (_rowActive == null) {
			throw new ConfigurationException(getClass().getSimpleName() + ": Parameter 'rowActive' is not configured");
		} else {
			indexActive = UtilsExcel.adaptAndCheckRowId(getClass(), _rowActive, "rowActive");
		}
		if (_rowStatusTest == null) {
			throw new ConfigurationException(getClass().getSimpleName() + ": Parameter 'rowStatusTest' is not configured");
		} else {
			indexStatusTest = UtilsExcel.adaptAndCheckRowId(getClass(), _rowStatusTest, "rowStatusTest");
		}
		if (_rowName == null) {
			indexName = null;
		} else {
			indexName = UtilsExcel.adaptAndCheckRowId(getClass(), _rowName, "rowName");
		}
		if (_rowDescription == null) {
			indexDescription = null;
		} else {
			indexDescription = UtilsExcel.adaptAndCheckRowId(getClass(), _rowDescription, "rowDescription");
		}
		if (_rowGroup == null) {
			indexGroup = null;
		} else {
			indexGroup = UtilsExcel.adaptAndCheckRowId(getClass(), _rowGroup, "rowGroup");
		}
		if (_rowOrder == null) {
			indexOrder = null;
		} else {
			indexOrder = UtilsExcel.adaptAndCheckRowId(getClass(), _rowOrder, "rowOrder");
		}
