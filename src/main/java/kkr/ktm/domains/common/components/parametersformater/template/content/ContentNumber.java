...
public class ContentNumber extends ContentBase implements Content {
	...
	public ContentNumber(Position position, Map<String, String> attributes, ExpressionParser expressionParser)
		...
		try {
			...
			// FORMAT
			//
			if (attributes.containsKey(ATTR_FORMAT)) {
				...
				try {
					...
					throw new ContentParseException(position, "" //
							...
							+ "[" + TAG + " " + ATTR_FORMAT + "=" + "\"" + attributeValue + "\"]" //
			...
			// EXPRESSION
			//
			if (attributes.containsKey(ATTR_EXPRESSION)) {
				String attributeValue = attributesLocal.remove(ATTR_EXPRESSION);

				if (expressionParser == null) {
					throw new ContentParseException(position, "" //
							+ "Expression parser must be configured " //
							+ "[" + TAG + " " + ATTR_EXPRESSION + "=" + "\"" + attributeValue + "\"]");
				}

				expression = expressionParser.parseExpression(attributeValue);
			} else {
				throw new ContentParseException(position, "Missing attribute [" + TAG + " " + ATTR_EXPRESSION + "]");
			}
