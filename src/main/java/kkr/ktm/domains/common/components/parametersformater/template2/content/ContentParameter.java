...
public class ContentParameter extends ContentBase implements Content {
	...
	public ContentParameter(Position position, Map<String, String> attributes) throws ContentParseException {
		...
		try {
			...
			// INDEXES
			//
			if (attributesLocal.containsKey(TagParameter.ATTR_INDEXES)) {
				String attributeValue = attributesLocal.remove(TagParameter.ATTR_INDEXES);
			...
			if (attributesLocal.containsKey(TagParameter.ATTR_FORMAT_TYPE)) {
				String attributeValue = attributesLocal.remove(TagParameter.ATTR_FORMAT_TYPE);
				try {
					formatType = FormatType.valueOf(attributeValue);
				} catch (Exception ex) {
					throw new ContentParseException(position,
							"Bad format-type of the attribute [" + TAG + " " + TagParameter.ATTR_FORMAT_TYPE + "]"
			...
			//
			{
				String attributeValue = attributesLocal.remove(TagParameter.ATTR_FORMAT);
				...
				try {
					...
					throw new ContentParseException(position,
							"Bad format of the attribute [" + TAG + " " + TagParameter.ATTR_FORMAT + "=\""
