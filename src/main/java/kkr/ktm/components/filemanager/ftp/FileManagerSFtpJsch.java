package kkr.ktm.components.filemanager.ftp;
...
public class FileManagerSFtpJsch extends FileManagerFtpApacheCommonsFwk implements
		FileManager {
	private static final Logger LOG = Logger
			.getLogger(FileManagerSFtpJsch.class);
		...
		try {
			...
			try {
				...
				if (traceDataFile != null) {
					File fileTrace = generateTraceFile(content, new Date(),
							filename, encoding);
					try {
				...
				throw new TechnicalException("Impossible to create the file: "
						+ filename + " in the remote directory: " + dir, ex);
